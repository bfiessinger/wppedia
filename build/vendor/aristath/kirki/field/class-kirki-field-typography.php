...
class Kirki_Field_Typography extends Kirki_Field
{
    ...
    public static function sanitize($value)
    {
        ...
        foreach ($value as $key => $val) {
            switch ($key) {
                ...
                case 'word-spacing':
                case 'line-height':
                    $value[$key] = '' === \trim($value[$key]) ? '' : sanitize_text_field($val);
                    break;
                case 'text-align':
                    if (!\in_array($val, array('', 'inherit', 'left', 'center', 'right', 'justify'), \true)) {
                        $value['text-align'] = '';
                    }
                    break;
                case 'text-transform':
                    if (!\in_array($val, array('', 'none', 'capitalize', 'uppercase', 'lowercase', 'initial', 'inherit'), \true)) {
                        $value['text-transform'] = '';
                    }
                    break;
                case 'text-decoration':
                    if (!\in_array($val, array('', 'none', 'underline', 'overline', 'line-through', 'initial', 'inherit'), \true)) {
                        $value['text-transform'] = '';
                    }
                    break;
                case 'color':
    ...
    protected function set_choices()
    {
        ...
        $this->choices = wp_parse_args($this->choices, array('variant' => array(), 'fonts' => array('standard' => array(), 'google' => array())));
    }
}
/**
 * Override field methods
 ...
 * Field overrides.
 */
\class_alias('WPPedia\\Container\\Kirki_Field_Typography', 'Kirki_Field_Typography', \false);
