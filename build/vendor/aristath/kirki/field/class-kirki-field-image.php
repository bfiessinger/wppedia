...
class Kirki_Field_Image extends Kirki_Field
{
    ...
    {
        $this->button_labels = wp_parse_args($this->button_labels, array('select' => esc_html__('Select image', 'kirki'), 'change' => esc_html__('Change image', 'kirki'), 'default' => esc_html__('Default', 'kirki'), 'remove' => esc_html__('Remove', 'kirki'), 'placeholder' => esc_html__('No image selected', 'kirki'), 'frame_title' => esc_html__('Select image', 'kirki'), 'frame_button' => esc_html__('Choose image', 'kirki')));
    }
    /**
     * Set the choices.
    ...
    public function sanitize($value)
    {
        if (isset($this->choices['save_as']) && 'array' === $this->choices['save_as']) {
            return array('id' => isset($value['id']) && '' !== $value['id'] ? (int) $value['id'] : '', 'url' => isset($value['url']) && '' !== $value['url'] ? esc_url_raw($value['url']) : '', 'width' => isset($value['width']) && '' !== $value['width'] ? (int) $value['width'] : '', 'height' => isset($value['height']) && '' !== $value['height'] ? (int) $value['height'] : '');
        }
 ...
 * Field overrides.
 */
\class_alias('WPPedia\\Container\\Kirki_Field_Image', 'Kirki_Field_Image', \false);
