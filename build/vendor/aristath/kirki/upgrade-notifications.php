 ...
 * @since       3.0.0
 */
if (!\function_exists('WPPedia\\Container\\kirki_show_upgrade_notification')) {
    /**
    ...
    function kirki_show_upgrade_notification($plugin_data, $response)
    {
        ...
        if (isset($response->upgrade_notice) && \strlen(\trim($response->upgrade_notice)) > 0) {
            ?>
			...
			<div class="kirki-upgrade-notification">
				<strong><?php 
            esc_html_e('Important Upgrade Notice:', 'kirki');
            ?></strong>
				<?php 
            $upgrade_notice = wp_strip_all_tags($response->upgrade_notice);
            ?>
				<?php 
            echo esc_html($upgrade_notice);
            ?>
			</div>
			<?php 
        }
