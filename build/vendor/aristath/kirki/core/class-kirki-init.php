...
class Kirki_Init
{
    ...
    public function register_control_types()
    {
        ...
        if (!\class_exists('WPPedia\\Container\\WP_Customize_Code_Editor_Control')) {
        ...
        $skip_control_types = apply_filters('kirki_control_types_exclude', array('Kirki_Control_Repeater', 'WP_Customize_Control'));
        foreach ($this->control_types as $control_type) {
            if (!\in_array($control_type, $skip_control_types, \true) && \class_exists($control_type)) {
    ...
    public function admin_notices()
    {
        ...
        $dismissed = get_user_meta(get_current_user_id(), 'kirki_fa_nag_dismissed', \true);
        if (\true === $dismissed || 1 === $dismissed || '1' === $dismissed) {
            return;
        }
        ?>
		<div class="notice notice-info is-dismissible">
			<p>
				<?php 
        esc_html_e('Your theme uses a Font Awesome field for icons. To avoid issues with missing icons on your frontend we recommend you install the official Font Awesome plugin.', 'kirki');
        ?>
			</p>
			<p>
				<a class="button button-primary" href="<?php 
        echo esc_url(admin_url('plugin-install.php?tab=plugin-information&plugin=font-awesome&TB_iframe=true&width=600&height=550'));
        ?>"><?php 
        esc_html_e('Install Plugin', 'kirki');
        ?></a>
				<a class="button button-secondary" href="<?php 
        echo esc_url(wp_nonce_url(admin_url('?dismiss-nag=font-awesome-kirki'), 'kirki-dismiss-nag', 'nonce'));
        ?>"><?php 
        esc_html_e('Don\'t show this again', 'kirki');
        ?></a>
			</p>
		</div>
		<?php 
    ...
    public function dismiss_nag()
    {
        if (isset($_GET['nonce']) && wp_verify_nonce($_GET['nonce'], 'kirki-dismiss-nag')) {
            ...
            if (get_current_user_id() && isset($_GET['dismiss-nag']) && 'font-awesome-kirki' === $_GET['dismiss-nag']) {
                update_user_meta(get_current_user_id(), 'kirki_fa_nag_dismissed', \true);
            }
        }
    }
    /**
    ...
    protected static function maybe_show_fontawesome_nag($args)
    {
        ...
        if (isset($args['type']) && \in_array($args['type'], array('fontawesome', 'kirki-fontawesome'), \true)) {
            // Skip check if theme has disabled FA enqueueing via a filter.
            if (!apply_filters('kirki_load_fontawesome', \true)) {
                return;
            }
            // If we got this far, we need to show the nag.
            self::$show_fa_nag = \true;
        }
    }
}
/**
 ...
 * Initialize Kirki
 */
\class_alias('WPPedia\\Container\\Kirki_Init', 'Kirki_Init', \false);
