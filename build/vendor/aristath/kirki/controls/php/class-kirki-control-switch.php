		...
		<div class="switch<# if ( data.choices['round'] ) { #> round<# } #>">
			...
			<label class="switch-label" for="switch_{{ data.id }}">
				<span class="switch-on">
					<# data.choices.on = data.choices.on || '<?php 
        esc_html_e('On', 'kirki');
        ?>' #>
					{{ data.choices.on }}
				</span>
				<span class="switch-off">
					<# data.choices.off = data.choices.off || '<?php 
        esc_html_e('Off', 'kirki');
        ?>' #>
					{{ data.choices.off }}
				</span>
			</label>
 ...
 * Switch control (modified checkbox).
 */
\class_alias('WPPedia\\Container\\Kirki_Control_Switch', 'Kirki_Control_Switch', \false);
