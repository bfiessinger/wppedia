...
class Kirki_Control_Repeater extends Kirki_Control_Base
{
    ...
    public function __construct($manager, $id, $args = array())
    {
        ...
        $this->row_label = array('type' => 'text', 'value' => esc_attr__('row', 'kirki'), 'field' => \false);
        // Validate row-labels.
        $this->row_label($args);
            ...
            if (isset($value['type'])) {
                switch ($value['type']) {
                    ...
                    case 'upload':
                        ...
                        $media_fields_to_filter[$key] = \true;
                        break;
                    case 'dropdown-pages':
                        // If the field is a dropdown-pages field then add it to args.
                        $dropdown = wp_dropdown_pages(array('name' => '', 'echo' => 0, 'show_option_none' => esc_html__('Select a Page', 'kirki'), 'option_none_value' => '0', 'selected' => ''));
    ...
    protected function render_content()
    {
        ...
        ?>
			<input type="hidden" {{{ data.inputAttrs }}} value="" <?php 
        echo wp_kses_post($this->get_link());
        ?> />
		</label>
		...
		<?php 
        if (isset($this->choices['limit'])) {
            ?>
			<?php 
            /* translators: %s represents the number of rows we're limiting the repeater to allow. */
            ?>
			<p class="limit"><?php 
            \printf(esc_html__('Limit: %s rows', 'kirki'), esc_html($this->choices['limit']));
            ?></p>
		<?php 
        }
        ?>
		<button class="button-secondary repeater-add"><?php 
        echo esc_html($this->button_label);
        ?></button>
    ...
    public function repeater_js_template()
    {
					...
					<# _.each( data, function( field, i ) { #>
						...
						<div class="repeater-field repeater-field-{{{ field.type }}} repeater-field-{{ field.id }}">
							...
							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
								...
								<figure class="kirki-image-attachment" data-placeholder="<?php 
        esc_attr_e('No Image Selected', 'kirki');
        ?>" >
									<# if ( field.default ) { #>
									...
									<# } else { #>
										<?php 
        esc_html_e('No Image Selected', 'kirki');
        ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php 
        esc_html_e('Remove', 'kirki');
        ?></button>
									<button type="button" class="button upload-button" data-label=" <?php 
        esc_attr_e('Add Image', 'kirki');
        ?>" data-alt-label="<?php 
        echo esc_attr_e('Change Image', 'kirki');
        ?>" >
										<# if ( field.default ) { #>
											<?php 
        esc_html_e('Change Image', 'kirki');
        ?>
										<# } else { #>
											<?php 
        esc_html_e('Add Image', 'kirki');
        ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
							...
							<# } else if ( field.type === 'upload' ) { #>
								...
								<figure class="kirki-file-attachment" data-placeholder="<?php 
        esc_attr_e('No File Selected', 'kirki');
        ?>" >
									<# if ( field.default ) { #>
        ...
        ?></button>
									<button type="button" class="button upload-button" data-label="<?php 
        esc_attr_e('Add File', 'kirki');
        ?>" data-alt-label="<?php 
        esc_attr_e('Change File', 'kirki');
        ?>">
										<# if ( field.default ) { #>
											<?php 
        esc_html_e('Change File', 'kirki');
        ?>
										<# } else { #>
											<?php 
        esc_html_e('Add File', 'kirki');
        ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
					...
					<# }); #>
					<button type="button" class="button-link repeater-row-remove"><?php 
        esc_html_e('Remove', 'kirki');
        ?></button>
				</div>
			</li>
 ...
 * Repeater control
 */
\class_alias('WPPedia\\Container\\Kirki_Control_Repeater', 'Kirki_Control_Repeater', \false);
