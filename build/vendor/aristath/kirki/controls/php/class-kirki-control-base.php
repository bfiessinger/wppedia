...
class Kirki_Control_Base extends WP_Customize_Control
{
    ...
    public function enqueue()
    {
        ...
        $suffix = '';
        $suffix .= !\defined('SCRIPT_DEBUG') || \true !== \SCRIPT_DEBUG ? '.min' : '';
        // The Kirki plugin URL.
        $kirki_url = trailingslashit(Kirki::$url);
        // Enqueue ColorPicker.
        wp_enqueue_script('wp-color-picker-alpha', trailingslashit(Kirki::$url) . 'assets/vendor/wp-color-picker-alpha/wp-color-picker-alpha.js', array('wp-color-picker', 'wp-i18n'), \KIRKI_VERSION, \true);
        wp_enqueue_style('wp-color-picker');
        // Enqueue selectWoo.
        wp_enqueue_script('selectWoo', trailingslashit(Kirki::$url) . 'assets/vendor/selectWoo/js/selectWoo.full.js', array('jquery'), '1.0.1', \true);
        ...
        // Enqueue the script.
        wp_enqueue_script('kirki-script', "{$kirki_url}controls/js/script{$suffix}.js", array('jquery', 'customize-base', 'wp-color-picker-alpha', 'selectWoo', 'jquery-ui-button', 'jquery-ui-datepicker'), \KIRKI_VERSION, \false);
        wp_localize_script('kirki-script', 'kirkiL10n', array('isScriptDebug' => \defined('SCRIPT_DEBUG') && \true === \SCRIPT_DEBUG, 'noFileSelected' => esc_html__('No File Selected', 'kirki'), 'remove' => esc_html__('Remove', 'kirki'), 'default' => esc_html__('Default', 'kirki'), 'selectFile' => esc_html__('Select File', 'kirki'), 'standardFonts' => esc_html__('Standard Fonts', 'kirki'), 'googleFonts' => esc_html__('Google Fonts', 'kirki'), 'defaultCSSValues' => esc_html__('CSS Defaults', 'kirki'), 'defaultBrowserFamily' => esc_html__('Default Browser Font-Family', 'kirki')));
        $suffix = \str_replace('.min', '', $suffix);
        // Enqueue the style.
        wp_enqueue_style('kirki-styles', "{$kirki_url}controls/css/styles{$suffix}.css", array(), \KIRKI_VERSION);
    }
    /**
 ...
 * A base for controls.
 */
\class_alias('WPPedia\\Container\\Kirki_Control_Base', 'Kirki_Control_Base', \false);
