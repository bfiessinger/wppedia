...
class Kirki_Modules_CSS
{
    ...
    public function init()
    {
        ...
        if (\defined('KIRKI_NO_OUTPUT') && \true === \KIRKI_NO_OUTPUT || isset($config['disable_output']) && \true === $config['disable_output']) {
            return;
        }
        ...
        if (!apply_filters('kirki_output_inline_styles', \true)) {
    ...
    {
        $args = array('action' => apply_filters('kirki_styles_action_handle', 'kirki-styles'));
        if (is_admin() && !is_customize_preview()) {
            $args['editor'] = '1';
        }
        // Enqueue the dynamic stylesheet.
        wp_enqueue_style('kirki-styles', add_query_arg($args, site_url()), array(), \KIRKI_VERSION);
    }
    /**
    ...
    public function print_styles_action()
    {
         ...
         */
        if (empty($_GET['action']) || apply_filters('kirki_styles_action_handle', 'kirki-styles') !== $_GET['action']) {
    ...
    public function print_styles()
    {
        ...
        foreach ($configs as $config_id => $args) {
            if (isset($args['disable_output']) && \true === $args['disable_output']) {
                continue;
            }
    ...
    public static function loop_controls($config_id)
    {
        ...
        foreach ($fields as $field) {
            ...
            if (\true === apply_filters("kirki_{$config_id}_css_skip_hidden", \true)) {
 ...
 * The Kirki_Modules_CSS object.
 */
\class_alias('WPPedia\\Container\\Kirki_Modules_CSS', 'Kirki_Modules_CSS', \false);
