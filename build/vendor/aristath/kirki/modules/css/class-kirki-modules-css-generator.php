...
final class Kirki_Modules_CSS_Generator
{
    ...
    public static function css($field)
    {
        ...
        $default_classnames = array('kirki-background' => 'Kirki_Output_Field_Background', 'kirki-dimensions' => 'Kirki_Output_Field_Dimensions', 'kirki-image' => 'Kirki_Output_Field_Image', 'kirki-typography' => 'Kirki_Output_Field_Typography', 'kirki-multicolor' => 'Kirki_Output_Field_Multicolor');
        ...
        if (\is_array($css)) {
            foreach ($css as $media_query => $elements) {
                foreach ($elements as $element => $style_array) {
                    foreach ($style_array as $property => $value) {
                        // Add -webkit-* and -moz-*.
                        if (\is_string($property) && \in_array($property, array('border-radius', 'box-shadow', 'box-sizing', 'text-shadow', 'transform', 'background-size', 'transition', 'transition-property'), \true)) {
                        ...
                        // Add -ms-* and -o-*.
                        if (\is_string($property) && \in_array($property, array('transform', 'background-size', 'transition', 'transition-property'), \true)) {
                            unset($css[$media_query][$element][$property]);
 ...
 * Handles CSS output.
 */
\class_alias('WPPedia\\Container\\Kirki_Modules_CSS_Generator', 'Kirki_Modules_CSS_Generator', \false);
