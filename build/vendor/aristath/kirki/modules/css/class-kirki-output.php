...
class Kirki_Output
{
            ...
            foreach ($output['pattern_replace'] as $search => $replace) {
                ...
                switch ($option_type) {
                    ...
                    case 'user_meta':
                        ...
                        if ($user_id) {
                            $replacement = get_user_meta($user_id, $replace, \true);
                        }
                        break;
                    default:
    ...
    protected function parse_output()
    {
        foreach ($this->output as $output) {
            ...
            if (is_admin() && !is_customize_preview() || isset($_GET['editor']) && '1' === $_GET['editor']) {
                // phpcs:ignore WordPress.Security.NonceVerification
                // Check if this is an admin style.
                if (!isset($output['context']) || !\in_array('editor', $output['context'], \true)) {
                    continue;
                }
            } elseif (isset($output['context']) && !\in_array('front', $output['context'], \true)) {
    ...
    protected function process_output($output, $value)
    {
        ...
        $accepts_multiple = array('background-image', 'background');
        if (\in_array($output['property'], $accepts_multiple, \true)) {
    ...
    {
        $properties = apply_filters("kirki_{$this->config_id}_output_property_classnames", array('font-family' => 'Kirki_Output_Property_Font_Family', 'background-image' => 'Kirki_Output_Property_Background_Image', 'background-position' => 'Kirki_Output_Property_Background_Position'));
 ...
 * Handles field CSS output.
 */
\class_alias('WPPedia\\Container\\Kirki_Output', 'Kirki_Output', \false);
