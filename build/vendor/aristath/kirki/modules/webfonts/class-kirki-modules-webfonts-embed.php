...
final class Kirki_Modules_Webfonts_Embed
{
    ...
    public function populate_fonts()
    {
        ...
        foreach ($this->googlefonts->fonts as $font => $weights) {
            ...
            $this->fonts_to_load[] = array('family' => $font, 'weights' => $weights);
        }
    }
    /**
    ...
    public function the_css()
    {
        foreach ($this->fonts_to_load as $font) {
            $family = \str_replace(' ', '+', \trim($font['family']));
            ...
            if (!\class_exists('WPPedia\\Container\\Kirki_Fonts_Downloader')) {
                include_once wp_normalize_path(\dirname(__FILE__) . '/class-kirki-fonts-downloader.php');
 ...
 * Manages the way Google Fonts are enqueued.
 */
\class_alias('WPPedia\\Container\\Kirki_Modules_Webfonts_Embed', 'Kirki_Modules_Webfonts_Embed', \false);
