...
class Kirki_Fonts_Downloader
{
    ...
    protected function get_local_files_from_css($css)
    {
        ...
        $change = \false;
        // If in the end this is true, we need to update the cache option.
        ...
        foreach ($font_files as $font_family => $files) {
            ...
            foreach ($files as $url) {
                ...
                if (!\function_exists('WPPedia\\Container\\download_url')) {
                ...
                $success = $this->get_filesystem()->move($tmp_path, $font_path, \true);
                if ($success) {
                    $stored[$url] = $font_path;
                    $change = \true;
                }
    ...
    public function get_files_from_css($css)
    {
        ...
        foreach ($font_faces as $font_face) {
            ...
            if (isset($matched_font_families[0]) && isset($matched_font_families[0][0])) {
                ...
                $font_family = \trim(\str_replace(array("'", ';'), '', $font_family));
    ...
    protected function get_filesystem()
    {
        ...
        if (!$wp_filesystem) {
            if (!\function_exists('WPPedia\\Container\\WP_Filesystem')) {
                require_once wp_normalize_path(\ABSPATH . '/wp-admin/includes/file.php');
            }
            WP_Filesystem();
        }
 ...
 * @since 3.1.0
 */
\class_alias('WPPedia\\Container\\Kirki_Fonts_Downloader', 'Kirki_Fonts_Downloader', \false);
