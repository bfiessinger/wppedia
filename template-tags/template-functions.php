...
function wppedia_navigation_link( string $term_slug ) {
	...
	if ( 'home' == $term_slug ) {
		...
		$link_url = ( wppedia_utils()->has_static_archive_page() ) ? get_permalink( wppedia_utils()->has_static_archive_page() ) : get_post_type_archive_link( 'wppedia_term' );
...
function wppedia_searchform() { ?>
	...
	<form <?php echo bf\wpPedia\template::getInstance()->get_search_form_attrs(); ?>>
