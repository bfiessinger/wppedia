	...
	<nav id="glossary-char-navigation">
		...
		<?php foreach ( wppedia_utils()->get_wiki_initial_letters(['hide_empty' => false]) as $slug => $initial ): ?>
			...
			?>
				...
				<a class="block px-3 py-2 bg-grey text-darkgrey" href="<?php echo get_post_type_archive_link('wp_pedia_term') . '#glossary-initial-' . strtoupper( \rawurlencode( $initial ) ); ?>" title="<?php echo sprintf(__('Glossary terms with initial character „%s“ (%d)', 'wppedia'), $initial, $initial_term->count); ?>"><?php echo $initial; ?></a>		
