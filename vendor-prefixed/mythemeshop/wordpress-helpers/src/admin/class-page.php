 ...
 */
namespace WPPedia_Vendor\MyThemeShop\Admin;
...
class Page
{
    ...
    public function init()
    {
        ...
        $hooks = ['admin_init' => ['callback' => 'save', 'condition' => !\is_null($this->onsave) && \is_callable($this->onsave)], 'admin_enqueue_scripts' => ['callback' => 'enqueue', 'condition' => !empty($this->assets)], 'admin_head' => ['callback' => 'contextual_help', 'condition' => !empty($this->help)], 'admin_body_class' => ['callback' => 'body_class', 'condition' => !empty($this->classes)]];
        foreach ($hooks as $hook => $data) {
            if (\true === $data['condition']) {
    ...
    private function enqueue_scripts()
    {
        ...
        foreach ($this->assets['scripts'] as $handle => $src) {
            wp_enqueue_script($handle, $src, null, null, \true);
        }
    }
    /**
     * Get tab content
