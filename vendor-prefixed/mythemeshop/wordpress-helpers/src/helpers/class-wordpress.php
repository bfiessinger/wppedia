...
use WPPedia_Vendor\MyThemeShop\Helpers\Str;
...
class WordPress
{
    ...
    {
        return wp_strip_all_tags(get_bloginfo('name'), \true);
    }
    /**
    ...
    {
        if ($post_type = Param::request('post_type')) {
            // phpcs:ignore
            return sanitize_key($post_type);
        }
        if ($post_id = Param::request('post_ID', 0, \FILTER_VALIDATE_INT)) {
            // phpcs:ignore
            return get_post_type($post_id);
        }
        // @codeCoverageIgnoreStart
        if ($post = Param::get('post')) {
            // phpcs:ignore
